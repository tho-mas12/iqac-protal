...
export default function Sidebar({ role, userName, departmentName, shift }: SidebarProps) {
  ...
  return (
    <aside
      className={`sticky top-0 h-screen overflow-y-auto shrink-0 bg-[#0d131f] text-slate-300 flex flex-col justify-between border-r border-[#192338] transition-all duration-300 select-none z-30 ${
        collapsed ? 'w-20' : 'w-64'
      }`}
    >
      {/* Top Header & Brand */}
      <div>
        ...
        <button
          ...
          className="absolute -right-3.5 top-7 w-7 h-7 bg-[#6d28d9] text-white rounded-full flex items-center justify-center shadow-lg hover:bg-purple-600 transition-colors z-40 border-2 border-[#0d131f]"
        ...
        <nav className="p-3 space-y-1.5 mt-2">
          {navItems.map((item) => {
            ...
            const isActive = pathname === item.href || (item.href !== '/director/dashboard' && item.href !== '/department/dashboard' && pathname.startsWith(item.href));
            ...
            return (
              ...
              >
                <Icon className={`w-5 h-5 shrink-0 ${isActive ? 'text-white' : 'text-slate-400 group-hover:text-purple-400'}`} />
                {!collapsed && (
